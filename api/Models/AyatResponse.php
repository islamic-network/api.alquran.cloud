...
use Api\Utils\Meta;
use Api\Utils\Request as RequestHelper;
...
class AyatResponse extends QuranResponse
{
     ...
     */
    public function __construct(EntityManager $em,$number = null, $edition = 'quran-simple', $all = false, $includeEdition = true, $includeSurat = true)
    {
        ...
        if ($this->edition->getFormat() == 'audio') {
            $this->edition = (new EditionResponse($this->entityManager, null, null, null, false))->getEditionByIdentifier('quran-uthmani');
            $this->audioEdition = (new EditionResponse($this->entityManager, null, null, null, false))->getEditionByIdentifier($edition);
        }
    ...
    {
        $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['edition' => $this->edition]);
    ...
    {
        $ayat = $this->entityManager->getRepository(Ayat::class)->findOneBy(['number' => $number, 'edition' => $this->edition]);
        if ($ayat) {
            $this->response = $this->prepare($ayat);
            $this->setCode(200);
            $this->setStatus('OK');
    ...
    {
        $ayat = $this->entityManager->getRepository(Ayat::class)->findOneBy(['surat' => $surat, 'numberInSurat' => $ayat, 'edition' => $this->edition]);
        if ($ayat) {
            $this->response = $this->prepare($ayat);
            $this->setCode(200);
            $this->setStatus('OK');
    ...
    public function loadBySurat($surat, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0 && $limit > 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['surat' => $surat, 'edition' => $this->edition], ['numberInSurat' => 'ASC'], $limit);
        } elseif ($offset ==0 && $limit == 0)  {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['surat' => $surat, 'edition' => $this->edition], ['numberInSurat' => 'ASC']);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['surat' => $surat, 'edition' => $this->edition], ['numberInSurat' => 'ASC'], $limit, $offset);
        }
        if ($ayat) {
            $this->response = $this->prepare($ayat);
            $this->setCode(200);
    ...
    public function loadByJuz($juz, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition], ['number' => 'ASC'], $limit);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition], ['number' => 'ASC'], $limit, $offset);
        }
        //$ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition]);
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
        $this->setStatus('OK');
    }
    ...
    public function loadByManzil($manzil, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['manzil' => $manzil, 'edition' => $this->edition], ['number' => 'ASC'], $limit);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['manzil' => $manzil, 'edition' => $this->edition], ['number' => 'ASC'], $limit, $offset);
        }
        //$ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition]);
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
    ...
    public function loadByPage($page, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['page' => $page, 'edition' => $this->edition], ['number' => 'ASC'], $limit);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['page' => $page, 'edition' => $this->edition], ['number' => 'ASC'], $limit, $offset);
        }
        //$ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition]);
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
    ...
    public function loadByRuku($ruku, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['ruku' => $ruku, 'edition' => $this->edition], ['number' => 'ASC'], $limit);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['ruku' => $ruku, 'edition' => $this->edition], ['number' => 'ASC'], $limit, $offset);
        }
        //$ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition]);
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
    ...
    {
        $q = $this->entityManager->createQuery("SELECT a FROM \Api\Entities\Doctrine\Primary\Ayat a WHERE a.sajda IS NOT NULL AND a.edition = {$this->edition->getId()}");
        $ayat = $q->getResult();
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
        $this->setStatus('OK');
    ...
    public function loadByHizbQuarter($hizbQuarter, $offset = null, $limit = null)
    {
        ...
        if ($offset == 0) {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['hizbQuarter' => $hizbQuarter, 'edition' => $this->edition], ['number' => 'ASC'], $limit);
        } else {
            $ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['hizbQuarter' => $hizbQuarter, 'edition' => $this->edition], ['number' => 'ASC'], $limit, $offset);
        }
        //$ayat = $this->entityManager->getRepository(Ayat::class)->findBy(['juz' => $juz, 'edition' => $this->edition]);
        $this->response = $this->prepare($ayat);
        $this->setCode(200);
    ...
    private function prepare($ayat)
    {
        ...
        if (is_array($ayat)) {
            foreach($ayat as $ayah) {
                ...
                $a['edition'] = (new EditionResponse($this->entityManager, $this->edition->getIdentifier()))->getResponse();
                $a['surah'] = (new SuratResponse($this->entityManager, $ayat->getSurat()->getId()))->getResponse();
    ...
    private function cacheEdition($ayah) {
        if(!isset($this->cache['edition'][$ayah->getEdition()->getId()])) {
            $this->cache['edition'][$ayah->getEdition()->getId()] = [
